...
function CandidateSection({ candidates, notify }: { candidates: CandidateMessage[]; notify: (message: string) => void }) { return <section className="candidate-section"><div className="section-heading"><div><p className="eyebrow">Needs a look</p><h3>Relevant inbox messages</h3></div><span className="source-note">{candidates.length} candidates</span></div><div className="message-list">{candidates.map((candidate) => <button className="message-row" key={candidate.id} onClick={() => notify(candidate.gmailThreadUrl ? `${candidate.sender}: ${candidate.subject}` : candidate.excerpt)}><span className="message-icon school-icon">✦</span><span><b>{candidate.subject || "Relevant Gmail message"}</b><small>{candidate.sender} · relevance {candidate.relevanceScore}</small></span><time>{candidate.receivedAt ? new Date(candidate.receivedAt).toLocaleDateString() : "Gmail"}</time><i>›</i></button>)}</div></section>; }
