...
export default function Home() {
  ...
  if (view === "dashboard") return <Dashboard events={events} notify={notify} />;
...
function Dashboard({ events, notify }: { events: ScheduleEvent[]; notify: (message: string) => void }) { const highlight = events[0]; return <main className="dashboard"><header className="dash-header"><Brand /><div className="header-right"><span className="inbox-pill"><i />Gmail connected</span><button className="profile" aria-label="Parent profile">L</button></div></header><div className="dashboard-content"><section className="welcome-row"><div><p className="eyebrow">Your inbox, simplified</p><h2>Your family’s week.</h2><p className="subhead">Here’s what Relay found in your recent updates.</p></div><div className="week-progress"><div className="sun-icon">☀</div><span>Week at a glance</span><strong>{events.length} things ahead</strong></div></section>{highlight && <section className="attention-card"><div className="attention-icon">✦</div><div><p className="eyebrow">Worth a look</p><h3>{highlight.title}</h3><p>{highlight.detail}</p></div><button className="detail-link" onClick={() => notify("Marked as seen")}>Got it <span>→</span></button></section>}<section className="schedule-section"><div className="section-heading"><div><p className="eyebrow">Your week</p><h3>Family schedule</h3></div><span className="source-note">From {events.length} Gmail updates</span></div>{events.length ? <div className="days-grid">{events.map((event, index) => <div className={`day-column ${index === 0 ? "today" : ""}`} key={`${event.day}-${event.title}`}><div className="day-head"><span>{event.day}</span><b>{event.date}</b></div><article className={`event-card ${event.style}`}><div className="event-time">{event.time}</div><div className="event-body"><span className="event-icon">{event.icon}</span><div><b>{event.title}</b><small>{event.detail}</small></div></div><span className="source-tag">{event.tag}</span></article></div>)}</div> : <p className="empty">No recent school, team, appointment, or reminder messages were found.</p>}</section><section className="inbox-section"><div className="section-heading"><div><p className="eyebrow">Recently found</p><h3>From your inbox</h3></div><button className="text-button" onClick={() => notify("These summaries came from the Gmail messages that informed this week")}>View source details</button></div><div className="message-list">{events.slice(0, 2).map((event) => <button className="message-row" key={event.title} onClick={() => notify(`Source: ${event.detail}`)}><span className={`message-icon ${event.style === "sports" ? "sports-icon" : "school-icon"}`}>{event.icon}</span><span><b>{event.title}</b><small>{event.detail}</small></span><time>{event.tag}</time><i>›</i></button>)}</div></section></div></main>; }
